...
const chartColors = ['#1f7a4d', '#f4b44f', '#496bba', '#b35f3a', '#7c5fb3', '#2d8b8b'];
...
const Result = () => {
  ...
  const totalVotes = Array.isArray(votes) ? votes.length : votes?.totalVotes || 0;
  const eligibleVoters = Array.isArray(event?.fileData) ? event.fileData.length : 0;
  const turnoutPercent = eligibleVoters > 0 ? (totalVotes / eligibleVoters) * 100 : 0;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      alert('Unable to download PDF. Please check that all candidate images are accessible and try again.');
    } finally {
  ...
  return (
    <main className="result-shell" ref={resultRef}>
      <section className="result-hero">
        ...
        <div className="result-hero-panel">
          ...
          <div className="result-actions">
            <button type="button" onClick={handlePrint}><FiPrinter /> Print</button>
      ...
      <section className="result-visual-grid">
        <div className="result-card result-donut-card">
          <div className="result-card-header">
            <span className="result-kicker"><FiPieChart /> Overall Share</span>
        ...
        <div className="result-card result-progress-card">
          <div className="result-card-header">
            <span className="result-kicker"><FiUsers /> Participation</span>
      ...
      <section className="result-card">
        <div className="result-card-header">
          <span className="result-kicker"><FiBarChart2 /> Candidate Graph</span>
              ...
              <article className="result-candidate-row" key={candidate.name}>
                <div className="result-candidate-identity">
                  {candidate.image ? (
                    ...
                    <span className="result-image-placeholder"><FiImage /></span>
                  )}
                  <div>
                    <strong>{candidate.name}</strong>
                    <span>{index === 0 && totalVotes > 0 ? 'Leading' : 'Final result share'}</span>
                  </div>
                </div>
                <div className="result-candidate-graph">
                  <span className="result-vote-count">{candidate.votes} votes received</span>
                  <div className="result-bar-track">
                    <div style={{ width: `${Math.min(candidate.percent, 100)}%` }} />
                  </div>
